...
class _MatchAndRevealScreenState extends State<MatchAndRevealScreen> {
  ...
  bool _isPartnerAVoting = true;
  bool _partnerADone = false;
  ...
  List<Map<String, dynamic>> _getMatches() {
    ...
    for (int i = 0; i < _cards.length; i++) {
      ...
      if (aVote != null && bVote != null) {
        if (aVote != VoteValue.no && bVote != VoteValue.no) {
          matches.add({
            'card': _cards[i],
            'aVote': aVote,
            'bVote': bVote,
  ...
  Widget build(BuildContext context) {
    ...
    if (_showReveal) {
      ...
      return _buildRevealScreen(l10n, primaryColor, matches);
    }

    if (_partnerADone && !_isPartnerAVoting) {
      ...
      body: Column(
        children: [
          GameProgressIndicator(
            ...
            color: primaryColor,
            label:
                '${l10n.translate('card')} ${_currentCardIndex + 1} ${l10n.ofLabel} ${_cards.length}',
            trailingWidget: Text(
              _isPartnerAVoting
                  ? '${_player1Name}\'s ${l10n.translate('turn')}'
                  : '${_player2Name}\'s ${l10n.translate('turn')}',
              style: TextStyle(
                fontSize: 13.fSize,
                color: primaryColor,
                fontWeight: FontWeight.bold,
          ...
          Padding(
            ...
            child: Text(
              _isPartnerAVoting
                  ? '${_player1Name}: ${l10n.translate('vote_privately')}'
                  : '${_player2Name}: ${l10n.translate('vote_privately')}',
              style: TextStyle(
                fontSize: 14.fSize,
                    ...
                    child: Column(
                      children: [
                        Container(
                          ...
                          decoration: BoxDecoration(
                            color: _getCardColor(currentCard.category)
                                .withOpacity(0.15),
                            borderRadius: BorderRadius.circular(20.adaptSize),
                          ),
                          child: Text(
                            currentCard.category?.toUpperCase() ??
                                l10n.translate('activity'),
                            style: TextStyle(
                              fontSize: 12.fSize,
                              fontWeight: FontWeight.bold,
                              color: _getCardColor(currentCard.category),
                        ...
                        Text(
                          currentCard.getLocalizedQuestion(
                        ...
                        if (currentCard.description != null) ...[
                          SizedBox(height: 16.h),
                          Text(
                            currentCard.getLocalizedDescription(
                              Localizations.localeOf(context).languageCode,
                            ) ?? '',
                            style: TextStyle(
                              fontSize: 15.fSize,
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: _buildVoteButton(
                    label: l10n.translate('maybe'),
                    icon: Icons.touch_app,
                    color: const Color(0xFFFF9800),
                    vote: VoteValue.maybe,
                  ),
                ),
                SizedBox(width: 12.adaptSize),
                Expanded(
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          Container(
            width: double.infinity,
            padding: EdgeInsets.fromLTRB(24.adaptSize, 20.adaptSize, 24.adaptSize, 20.adaptSize),
                          ...
                          children: [
                            Row(
                              children: [
                                ...
                                Spacer(),
                                Icon(Icons.favorite,
                                    color: primaryColor, size: 18.adaptSize),
                              ],
                            ),
                            SizedBox(height: 12.h),
                            Text(
                              card.getLocalizedQuestion(
                                Localizations.localeOf(context).languageCode,
                              ),
                              style: TextStyle(
                                fontSize: 16.fSize,
  ...
  Widget _buildVoteChip(String name, VoteValue vote) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: _getVoteColor(vote).withOpacity(0.15),
        borderRadius: BorderRadius.circular(12.adaptSize),
      ),
      child: Row(
